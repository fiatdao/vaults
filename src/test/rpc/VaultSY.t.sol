...
import {WAD, add, sub, mul, wmul, wdiv} from "fiat/utils/Math.sol";
...
contract VaultSY_ModifyPositionCollateralizationTest is DSTest, ERC1155Holder {
    ...
    function test_unwrap_amount_principal_increases() public {
        ...
        hevm.store(address(controller), bytes32(uint256(9)), bytes32(uint256(wdiv(fee, mul(2, WAD)))));
    ...
    function test_unwrap_amount_principal_decreases() public {
        ...
        // increases fee
        (uint256 prePrincipal, uint256 preConversion, uint128 maturity, , ) = vault.bonds(bondId);
        hevm.store(address(controller), bytes32(uint256(9)), bytes32(uint256(wmul(fee, mul(2, WAD)))));
        vault.updateBond(bondId);
        (uint256 principal, uint256 conversion, , , ) = vault.bonds(bondId);
    ...
    function test_exit_transfers_tokens(
        ...
        assertEq(vault.balanceOf(owner, tokenId), amount);
    ...
    function testFail_enter_amount_cannot_be_casted(uint256 amount) public {
        ...
        vault.setApprovalForAll(address(vault), true);
        hevm.store(address(vault), keccak256(abi.encode(me, keccak256(abi.encode(0, uint256(1))))), bytes32(amount));
    ...
    function testFail_exit_amount_cannot_be_casted(uint256 amount) public {
        ...
        hevm.store(
            address(vault),
            keccak256(abi.encode(me, keccak256(abi.encode(0, uint256(1))))),
            bytes32(MAX_AMOUNT)
        );
